...
- 同时搜索标题、正文与标签
- 在卡片上一键复制完整提示词
- 为每条提示词添加本地效果图片，支持卡片预览与大图查看
- 收藏、编辑和删除内容
- 明亮 / 暗夜模式切换并记住偏好
- 导出、导入包含效果图片的 JSON 本地备份
- Windows 10/11 与 macOS Apple/Intel 双架构支持

## 数据与隐私

桌面版通过系统 WebView 的本地存储保存文字，并通过本地 IndexedDB 保存效果图片，没有远程数据库、用户账户、遥测或广告。卸载软件或清理应用数据前，请先在软件左下角导出 JSON 备份。
...
当前版本暂未购买商业代码签名证书。系统可能显示开发者来源提醒：Windows 可在“更多信息”中继续，macOS 可在“系统设置 → 隐私与安全性”中确认打开。
